...
class CountryPilotsNavigation extends React.Component {
  componentDidMount() {
    if (this.props.countryPilots.length) {
      this.positionSelected();
    }
  }

  componentDidUpdate(prevProps, prevState) {
    if (prevProps.countryPilots.length !== this.props.countryPilots.length) {
      this.positionSelected();
    }
  }
  ...
  render() {
    const { countryPilots } = this.props;
...
CountryPilotsNavigation.propTypes = {
  countryPilots: T.array
};

export default CountryPilotsNavigation;
